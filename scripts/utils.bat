...
rem TODO: not sure what "call %*" do exactly. from my understanding it enables me to call the
rem     functions defined in this file from other files. how?! I'm not sure.
...
rem %ERRORLEVEL%: integer number returned by the child process when it terminates. it will equal
rem     0 if process succeeded and 1 or greated if process encountered an error.
exit /B %ERRORLEVEL%
...
:clang_build_library
  ...
  rem TODO: not sure why do we need enabledelayedexpansion
  ...
  rem list all c files in the "folder" directory
  rem for /R: for all files in the current path
  set c_filenames=
  for /R %%f in (*.c) do (
    rem TODO: not sure what variables between two "!" means
